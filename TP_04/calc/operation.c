...
int analyse_action(char *str_action) {
    ...
    if (((str_action[0] >= '1') && (str_action[0] <= '9')) ||
        (str_action[0] == '-')) {
        return 1;
    }
...
void modulo(void) {
    ...
    display_transition_2(a);
    display_transition_1(b);
    display_operation('m'); /* display_operation('%'); error when % */
    display_stack();
...
void facto(void) {
    ...
    display_operation('+'); /* display_operation('!'); error when % */
    printf("ici\n");
    display_stack();
    ...
    clear_window();
    display_stack();
    display_transition_1(a);
    display_action_msg("Push the factorial");
    actualise_window();
    pause_keyboard();

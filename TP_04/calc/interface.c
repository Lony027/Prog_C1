...
void create_windows(void) {
    MLV_create_window("Desk Calculator", NULL, WIDTH_WINDOWS_PIX,
                      HEIGHT_WINDOWS_PIX);
}

void free_windows(void) { MLV_free_window(); }

void actualise_window(void) { MLV_actualise_window(); }

void clear_window(void) { MLV_clear_window(MLV_rgba(0, 0, 0, 255)); }

void pause_action(void) { MLV_wait_milliseconds(WAIT_TIME_MILLISEC); }

void pause_keyboard(void) { MLV_wait_keyboard(NULL, NULL, NULL); }
...
void display_stack(void) {
    ...
    MLV_draw_line(20 + 80, 20, 20 + 80, 20 + (16 * 30),
                  MLV_rgba(255, 255, 255, 255));
    MLV_draw_line(20, 20 + (16 * 30), 20 + 80, 20 + (16 * 30),
                  MLV_rgba(255, 255, 255, 255));

    /* Draw the element in the stack. */
    for (i = 0; i < nb_elem; i++) {
        MLV_draw_line(20, 20 + ((15 - i) * 30), 20 + 80, 20 + ((15 - i) * 30),
                      MLV_rgba(255, 255, 255, 255));
        MLV_draw_filled_rectangle(23, 23 + ((15 - i) * 30), 75, 25,
                                  MLV_rgba(255, 55, 55, 255));
        length_char = sprintf(number, "%d", stack_get_element(i));
        MLV_draw_text(61 - (4 * length_char), 28 + ((15 - i) * 30), number,
                      MLV_rgba(255, 255, 255, 255));
    }
}
...
void display_transition_1(int number) {
    ...
    MLV_draw_text(288 - (4 * length_char), 255, msg,
                  MLV_rgba(255, 255, 255, 255));
}
...
void display_transition_2(int number) {
    ...
    length_char = sprintf(msg, "%d", number);
    MLV_draw_text(688 - (4 * length_char), 255, msg,
                  MLV_rgba(255, 255, 255, 255));
}
...
void get_user_action(char *str_action) {
    ...
    MLV_wait_input_box(300, 300, 300, 100, MLV_rgba(255, 255, 255, 255),
                       MLV_rgba(255, 255, 255, 255), MLV_rgba(255, 55, 55, 255),
                       "Next action : ", &str_action_adr);
